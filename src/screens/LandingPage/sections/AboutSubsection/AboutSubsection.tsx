...
import { TeamMemberCard, TeamMember } from "../../../../components/TeamMemberCard/TeamMemberCard";

const teamMembers: TeamMember[] = [
  {
    quote:
      "I build software solutions that make a difference.",
    name: "Goodness Obaje",
    role: "Director",
    isDark: true,
  ...
  {
    quote:
      "As the Lead Backend Developer, I make sure our systems are fast, secure, and scalable.",
    name: "Akachi Osuji",
    role: "Backend Dev",
    isDark: false,
  ...
  {
    quote:
      "I enjoy building backend systems that make everyday life easier.",
    name: "Nnamdi Nwaocha",
    role: "Backend Dev",
    isDark: false,
...
export const AboutSubsection = (): React.ReactElement => {
  ...
  const renderTeamContent = () => (
    <div className="flex flex-col items-center gap-12">
      <header className="flex flex-col w-[717px] items-center justify-center gap-2 relative flex-[0_0_auto]">
        <div className="flex flex-col items-center justify-center gap-2 relative self-stretch w-full flex-[0_0_auto]">
          ...
          <p className="relative w-[569px] opacity-50 [font-family:'Sora',Helvetica] font-normal text-neutral-900 text-base text-center tracking-[-0.64px] leading-[26px]">
            Melfi is made up of amazing smart team that sit on a round table,
            analyze your product and bring the best to your business.
          </p>
        </div>
      </header>
      ...
      <div 
        ref={scrollRef}
        className="flex gap-6 overflow-x-auto scrollbar-hide relative w-full"
      ...
      >
        ...
        {teamMembers.map((member, index) => (
          <TeamMemberCard key={`duplicate-${index}`} member={member} index={index + teamMembers.length} />
        ))}
      </div>
      ...
      <div className="inline-flex items-center gap-3 relative flex-[0_0_auto]">
        {paginationDots.map((dot, index) => (
          <div
            key={index}
            className={`relative h-3 rounded-[30px] ${
              dot.active ? "w-[27px] bg-[#8c8c8c]" : "w-3 bg-[#e4e4e4]"
            }`}
          />
        ))}
      </div>
    ...
    <div className="flex flex-col items-center gap-12 max-w-[1248px] mx-auto">
      <header className="flex flex-col items-center justify-center gap-2">
        ...
        <p className="opacity-50 [font-family:'Sora',Helvetica] font-normal text-neutral-900 text-base text-center tracking-[-0.64px] leading-[26px] max-w-[569px]">
          We've had the privilege of working with amazing companies and startups 
  ...
  return (
    <section id="about" className="relative w-full bg-white py-24 overflow-hidden scroll-mt-28">
      ...
      <div className="top-[525px] left-[-158px] w-[478px] h-[162px] -rotate-90 absolute bg-[linear-gradient(180deg,rgba(255,255,255,1)_42%,rgba(255,255,255,0)_100%)]" />
      <div className="absolute top-[514px] right-[-168px] w-[499px] h-[162px] -rotate-90 bg-[linear-gradient(180deg,rgba(255,255,255,1)_42%,rgba(255,255,255,0)_100%)]" />
    </section>
  );
};
